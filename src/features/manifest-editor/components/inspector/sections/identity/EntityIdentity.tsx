...
import { ManifestEntity, OMEGA_Manifest, OmegaNode } from '@/omega-ui-core/types/manifest';
...
export default function EntityIdentity({ entity, rootManifest, rootTree, onUpdate, onHelp, isHighlighted }: EntityIdentityProps) {
  const model = getInspectorModel(entity, rootTree, rootManifest?.ui?.moduleTemplates);
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input 
              ...
              disabled={!('label' in entity) || model.governance?.['label'] === 'locked'}
              className={`w-full bg-black/5 border ${isHighlighted('label') ? 'border-amber-500 ring-1 ring-amber-500 animate-pulse' : 'wb-outline'} rounded-xs px-3 py-2 text-[10px] font-bold wb-text outline-none focus:border-primary/40 transition-all [color-scheme:dark] ${(!('label' in entity) || model.governance?.['label'] === 'locked') ? 'opacity-50 cursor-not-allowed' : ''}`}
            />
          </div>
        </div>
      </div>
