...
export default function StylingSection({ item, onUpdate }: StylingSectionProps) {
  ...
  return (
    <section className="space-y-4">
      <div className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest border-b border-outline/10 pb-2 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="space-y-1">
          <label className="text-[8px] text-foreground/60 uppercase font-bold tracking-tighter">Official Variant</label>
          <select 
            value={pres.variant || 'A'} 
            onChange={(e) => onUpdate({ presentation: { ...pres, variant: e.target.value } })}
            className="w-full bg-black/40 border border-outline rounded-sm p-2 text-[10px] font-bold text-primary outline-none"
          >
            {variants.map(v => (
              <option key={v.id} value={v.id}>{v.label}</option>
            ))}
          </select>
          <p className="text-[7px] text-foreground/30 italic mt-1">Note: Variants define both physical dimensions and color-coding.</p>
        </div>

        {/* UNIT & PRECISION */}
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-[8px] text-foreground/60 uppercase font-bold tracking-tighter">Unit Suffix</label>
            <input 
              type="text" 
              value={pres.unit || ''} 
              onChange={(e) => onUpdate({ presentation: { ...pres, unit: e.target.value } })}
              placeholder="Hz, dB, %"
              className="w-full bg-black/40 border border-outline rounded-sm p-2 text-[10px] font-mono text-foreground outline-none focus:border-primary/40"
            />
          </div>
          <div className="space-y-1">
            ...
            <input 
              ...
              className="w-full bg-black/40 border border-outline rounded-sm p-2 text-[10px] font-mono text-foreground outline-none"
            />
          </div>
        </div>
