...
export default function KnobPrimitive({ value, variant, skin, isMain, isSelected, onValueChange, onClick }: any) {
  const parts = (variant || 'B_cyan').split('_');
  const size = parts[0] || 'B';
  const colorId = parts[1] || 'cyan';
  
  const colorMap: any = { cyan: '#00f2ff', red: '#ff4444', orange: '#ff8800', green: '#00ff88', white: '#ffffff' };
  const color = colorMap[colorId] || '#00f0ff';
  const diameter = { A: 32, B: 24, C: 16, D: 12 }[size as 'A'|'B'|'C'|'D'] || 24;
